...
import jp.co.fujielectric.fss.data.VotUploadPolicyNameResult;
    ...
    public Tuple<String, String> postUploadFile(String filename, InputStream is, long length) throws IOException, FssException {
        try{
            ...
            try(CloseableHttpClient httpClient = HttpClientBuilder.create()
                ...
                //VotiroのパラメータをSetting.propertiesから取得
                String votParam = CommonUtil.getSetting("votiroParameter");                
                // requestの作成
                postUrl = CommonUtil.createVotiroUrl() + "upload/file" + "?fileName=" + URLEncoder.encode(filename, "UTF-8") + votParam;
                HttpPost httpPost = new HttpPost(postUrl);
                LOG.debug("#Votiro Upload postUrl:{}", postUrl);

                //HTTPヘッダをSetting.propertiesから取得 (v2.2.5e)
                try {
                    String votHttpHeader = CommonUtil.getSetting("votiroUploadHttpHeader");
                    if(!StringUtils.isEmpty(votHttpHeader)){
                        //複数設定の場合、区切り文字"&"で区切られているので分割する
                        String[] headerSplit = votHttpHeader.split("&");
                        for(int i=0; i<headerSplit.length; i++){
                            String header = headerSplit[i];
                            //1つ目の":"を、項目名と値の区切りとみなして分割する
                            int index = header.indexOf(":");
                            if(index > 0){
                                String headerItem  = header.substring(0, index);    //項目
                                String headerValue = header.substring(index + 1);   //値
                                //Httpヘッダとして登録
                                httpPost.setHeader(headerItem, headerValue);
                                LOG.debug("#Votiro UploadHttpHeader:[{}] Item:[{}], Value:[{}]", i, headerItem,headerValue);
                            }
                        }
                    }else{
                        //Setting.propertiesから取得できなかった場合はヘッダ追加しない
                    }
                } catch (Exception e) {
                    LOG.warn("#Votiroアップロード HTTPヘッダーの設定に失敗しました。[{}]", e.toString());
                }
                ...
                try(CloseableHttpResponse response = httpClient.execute(httpPost)){
                    ...
                    if (statusCode == HttpStatus.SC_OK) {
                        ...
                        // リターン値の解析 
                        //[v2.2.5c] 2019/11/12. Votiroパラメータからリザルトタイプ（PolicyRules/PlicyName）を切替る
                        String requestId;
                        ObjectMapper mapper = new ObjectMapper();
                        if(StringUtils.containsIgnoreCase(votParam, "&PolicyName")){
                            //PolicyName使用パターン    [v2.2.5c]
                            VotUploadPolicyNameResult vur = mapper.readValue(
                                    EntityUtils.toString(entity, StandardCharsets.UTF_8),
                                    VotUploadPolicyNameResult.class);
                            requestId = vur.RequestID;  //リクエストID取得
                            LOG.trace("#Votiro Upload Return AllValues.（PlicyName) :"
                            ...
                            requestId = vur.RequestID;  //リクエストID取得
                            LOG.trace("#Votiro Upload Return AllValues. (PlicyRules):"
                                    + "{}", ReflectionToStringBuilder.toString(vur, ToStringStyle.MULTI_LINE_STYLE));
                        }                      
                        tuple.setValue1(requestId);
