...
//            }
            ...
            if (sendInfo.getSendFiles().isEmpty()) {
                ...
                if (mailSanitizeFlg) {                                          // メール本体無害化判定
                    ...
                    && FileUtil.checkPw(sendFolder, LOG, itemHelper)) { // パスワード付きファイル有無判定
                
                //MAC-ZIP変換 [v2.2.5]
                sanitizeHelper.convMacZip(sendInfo);
    ...
    private boolean renameZipIncludeFiles(SendInfo sendInfo, String targetPath) {
        ...
        for(SendFile sendFile: sendInfo.getSendFiles()){
            ...
            File file = new File(sendFile.getFilePath());
...
//        sendInfoService.create(sendInfo);                                     // ここではDB登録しない
    ...
    //                    sendFileService.create(sendFile);                         // ここではDB登録しない
                        ...
                        //高圧縮ZIPファイル（ZipBomb）判定結果をfilepassにセットする(ZipBomb:"1", それ以外:"0") [v2.2.5d]
                        boolean isZipBomb = ZipUtil.isZipBomb(file.getPath(), LOG, itemHelper);
                        VerifyUtil.outputUtLog(LOG, "#v2.2.5d#ZipBomb#", false, "IsZipBomb:%b, File:%s", isZipBomb, file.getName());            
                        if(isZipBomb){
                            LOG.warn("#! insertSendFiles() ZipBomb. (SendInfo.ID:{}, SendFIle.ID:{}, FileName:{})",
                                    sendInfo.getId(), sendFile.getId(), sendFile.getFileName());
                        }
                        sendFile.setIsZipBomb(isZipBomb);
                    }
                }
            }
        }finally{
