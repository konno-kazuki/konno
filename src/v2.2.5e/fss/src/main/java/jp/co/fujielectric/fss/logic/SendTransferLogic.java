...
//            //TODO :::UT:::End
            ...
            if (true) {
                ...
                if(checkPasswordFlgByConfig(sendInfo)){
                    ...
                    hasPwFile = FileUtil.checkPw(CommonUtil.getFolderSend(sendInfo, false, false), LOG, itemHelper);  //例外発生しても内部でログ出力して処理継続
                }
                
                ///送信先アドレス情報
                InternetAddress[] envelopeToList = InternetAddress.parse(Optional.ofNullable(sendInfo.getToAddress()).orElse(""));
                //MAC-ZIP文字コード変換 [v2.2.5]
                convertMacZip(sendInfo, envelopeToList);
                //送信先毎の処理
...
//                //TODO :::UT:::End 
   ...
   /**
     * MAC-ZIP文字コード変換 [v2.2.5]
     * 送信先から無害化対象と判断された場合のみMAC-ZIP変換をする。
     * @param sendInfo
     * @param envelopeToList
     * @return 
     */
    private boolean convertMacZip(SendInfo sendInfo, InternetAddress[] envelopeToList)
    {
        boolean flgConv = false;        //変換したかどうか
        boolean flgSanitize = false;    //無害化対象かどうか
        try {
            if(envelopeToList == null)
                return false;
            //送信先から無害化対象（変換対象）があるかどうか判定
            for (InternetAddress envelopeTo : envelopeToList) {
                //庁内/庁外判定
                boolean isLgwanAddress = (mailManager.isMyDomain(envelopeTo.getAddress()) 
                        || mailManager.isLgDomain(envelopeTo.getAddress()));
                if (isLgwanAddress && !sendInfo.isSectionLgwan()) { // FIXME: 無害化対象の条件検討
                     //庁内向けもしくは庁外(LG)向け、かつインターネット側の場合
                     flgSanitize = true;    //無害化対象と判断⇒MAC-ZIP対象
                     break;
                }                        
            }
            //変換対象の場合、MAC-ZIP変換を実施
            if(flgSanitize){
                flgConv = sanitizeHelper.convMacZip(sendInfo);                    
            }
            return flgConv;
        } catch (Exception e) {
            //例外発生した場合、エラーログを出力して継続。（既存処理に影響がでないように）
            LOG.error("#! MAC-ZIP変換に失敗しました。 SendInfoID:{}, Err:{}", sendInfo.getId(), e.toString(), e);
            return false;
        }
    }
