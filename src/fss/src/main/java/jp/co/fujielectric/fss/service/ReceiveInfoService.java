...
public class ReceiveInfoService {
     ...
     * @return 受信履歴一覧
     */
    public List<ReceiveInfo> findForReceiveHistory(
            String receiveUserId,
            String receiveMailAddress,
            Date dateFrom, 
            Date dateTo) 
    {
        String queryName = ReceiveInfo.NAMED_QUEUE_FIND_RECEIVEHISTORY;
        TypedQuery<ReceiveInfo> query = em.createNamedQuery(queryName, ReceiveInfo.class);
     ...
     * @return 原本履歴一覧
     */
    public List<ReceiveInfo> findForOriginalHistory(
            String receiveUserId,
            String receiveMailAddress,
            Date dateFrom,
            Date dateTo, 
            int limit) 
    {
        String queryName = ReceiveInfo.NAMED_QUEUE_FIND_ORIGINALHISTORY;
        TypedQuery<ReceiveInfo> query = em.createNamedQuery(queryName, ReceiveInfo.class);
        query.setParameter("receiveMailAddress", receiveMailAddress);
        query.setParameter("dateFrom", dateFrom);
        query.setParameter("dateTo", dateTo);
