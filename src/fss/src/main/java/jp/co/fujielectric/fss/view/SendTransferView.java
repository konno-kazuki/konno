...
public class SendTransferView extends SendCommonView implements Serializable {
    ...
    @Inject
    protected UploadGroupInfoManager ugiManager;
...
//                errReason = FileInfoBean.FileErrorReason.FileNameLengthOver;
                ...
                //エラーメッセージ「ファイル名が長いため送信されません。ファイル名は８０文字未満として下さい。」
                errMsg = itemHelper.findDispMessageStr(ERR_FILE_SEND_TRANSFER_FILENAME_LONG, Item.FUNC_COMMON);
                //チェックエラーの例外として対象外処理をする
            ...
            try (InputStream is = upldFile.getInputstream()) {
                ...
                //チェックエラー以外の例外として対象外処理をする
                throw new RuntimeException("アップロードファイルの一時保存に失敗しました。");
            }
            //念の為、保存できているかチェック
            if(!objFile.exists())
                throw new RuntimeException("アップロードファイルの一時保存に失敗しました。");
...
//                    errReason = FileInfoBean.FileErrorReason.FileNameLengthOverInZip;
                    ...
                    //エラーメッセージ「ファイル名が長いファイルが含まれるため送信されません。ファイル名は８０文字未満として下さい。」
                    errMsg = itemHelper.findDispMessageStr(ERR_FILE_SEND_TRANSFER_ZIPFILENAME_LONG, Item.FUNC_COMMON);                                                        
                    //チェックエラーの例外として対象外処理をする
                    throw new FssException(errMsg);                            
                }
            }

            //高圧縮ZIPファイル（ZipBomb）は対象外とする [v2.2.5d]
            boolean isZipBomb = ZipUtil.isZipBomb(objFile.getPath(), LOG, itemHelper);
            VerifyUtil.outputUtLog(LOG, "#v2.2.5d#ZipBomb#", false, "IsZipBomb:%b, File:%s", isZipBomb, objFile.getName());
            if(isZipBomb){
                //高圧縮ZIPファイル（ZipBomb）の場合、エラー情報をセットする
//                errReason = FileInfoBean.FileErrorReason.ZipBomb;                    
                ...
                LOG.warn("#!ファイル送信画面で選択されたZIPが高圧縮ZIPファイル（ZipBomb）と判断されました。"
                            +"（RegionId:{}, UserId:{}, OnetimeId:{}, ファイル名：{}, zipFilesCountLimit:{}, zipFilesSizeLimit:{}）"
                        ,commonBean.getRegionId(), commonBean.getUserId(),  commonBean.getOnetimeId(), fname, zipFilesCountLimit, zipFilesSizeLimit); 

                //エラーメッセージ「上限を超えたZIPファイルのため送信されません。ZIPファイルに含まれるファイル数を%dファイル以下、合計のファイルサイズを%dMB以下としてください。」
                errMsg = itemHelper.findDispMessageStr(Item.ErrMsgItemKey.ERR_ZIP_BOMB_FILE, Item.FUNC_COMMON, zipFilesCountLimit, zipFilesSizeLimit);
                //チェックエラーの例外として対象外処理をする
                throw new FssException(errMsg);                            
            }
...
//            info.setFileErrorReason(errReason);
            ...
            //Internet側のみのファイルチェック（LGWAN側であれば、全ファイル許可する）
            if (!CommonUtil.isSectionLgwan()) {
                //ファイルチェック、エラーメッセージのセット
                if(chkFile(info) == false){
                    //チェックエラーの例外として対象外処理をする
                    throw new FssException(info.getErrMsg());                            
                }
            }
            fileInfoList.add(info);
        } catch (Throwable e){
            //チェックエラー、例外発生による送信対象外処理
            FileInfoBean info = new FileInfoBean(objFile);
            if(e instanceof FssException){
                //送信対象外とするチェックエラーの場合
                //チェック時に取得したエラーメッセージをセットする
                info.setErrMsg(e.getMessage());                
            }else{
                //チェックエラー以外の例外が発生した場合
                LOG.error("ファイル送信画面。ファイル選択に失敗しました。", e);
                info.setErrMsg("ファイルのアップロードに失敗しました。");
            }           
            //アップロードされたファイルが保存されていたら削除する
            if(objFile.exists()){
                objFile.delete();                
            }
            info.setSize(0);    //サイズを0とする(0=対象外として判定するため）
            fileInfoList.add(info);
        }
    }
    ...
    public String getActionConf() {
        ...
        //TODO :::UT:::Start v2.2.5a SyncFile テスト用処理（テストモード時のみ実行）
        if(VerifyUtil.UT_MODE){
            if(sendInfo.getContent().contains("#UTMODE#ON#")){
                VerifyUtil.flgUtTmpCancel = false;   //一時的なテストコードキャンセルを解除
                return null;
            }
            if(sendInfo.getContent().contains("#UTMODE#OFF#")){
                VerifyUtil.flgUtTmpCancel = true;   //一時的なテストコードキャンセルをセット
                return null;
            }
            
            //ファイル同期テスト
            if(VerifyUtil.chkUTKey(sendInfo.getContent(),"#UT225#","SYNCTEST")){
                if(VerifyUtil.chkUTKey(sendInfo.getContent(),"#UT225#","CANCEL")){
                    //一次的にテストコードをキャンセルして、通常処理で呼出す。
                    VerifyUtil.flgUtTmpCancel = true;
                }
                //ファイル同期テスト
                if(fileInfoList.size() > 0){
                    syncFilesService.syncFile(fileInfoList.get(0).getFilePath());
                }
                VerifyUtil.flgUtTmpCancel = false;   //一時的なテストコードキャンセルを解除
                return null;
            }
     ...
     *
     * @param info
     * @return 送信要否（true:送信対象、 false:送信対象外）
     */
    protected boolean chkFile(FileInfoBean info) {
        StringBuilder errMsg = new StringBuilder(); //大容量ファイルチェックのエラーメッセージがセットされる変数
                
        //大容量ファイルチェック ファイル本体（PDF,OFFICEファイル） [v2.2.6] 
        if( this.nonSanitizeFlg==false || this.passwordUnlockFlg==true){
            if(FileUtil.checkIsFileSizeOver(info.getFileName(), info.getSize(), false, errMsg, LOG, itemHelper)==true){
                //サイズ上限チェックでサイズオーバーと判定された場合はパスワードチェックせず、パスワード無しの判定とする
                LOG.warn("# ファイルサイズオーバー　パスワード処理スキップ対象（fname:{}, fsize:{}）", info.getFileName(), info.getSize());
                // メッセージ
                info.setErrMsg(errMsg.toString());
                
                return true;    //パスワード関連処理はしないが送信対象として返す
            }
        }
        
        if(this.nonSanitizeFlg==false) {
            //拡張子のチェック
            ...
            if (fileExcludeSufList.contains(suffix) || !fileIncludeSufList.contains(suffix)) {
                // メッセージ　"対象外ファイルのため送信されません。"
                info.setErrMsg(itemHelper.findDispMessageStr(Item.ErrMsgItemKey.FILE_ENABLE_SEND_EXCLUDE, funcId));

                return false;   //false (対象外）として返す
            }
        }
        
        if(this.nonSanitizeFlg==false && this.passwordUnlockFlg==false && info.getSize() > 0) {
            //パスワード付かチェック
            if (FileUtil.checkPw(info.getFile(), false, LOG, itemHelper)) {
                // メッセージ　パスワード付のため送信されません。
        ...
        //大容量ファイルチェック ZIP内ファイル（PDF,OFFICEファイル） [v2.2.6]  
        if( this.nonSanitizeFlg==false || this.passwordUnlockFlg==true){
            if(FileUtil.checkIsFileSizeOverZip(info.getFilePath(), errMsg, LOG, itemHelper)==true){
                //サイズ上限チェックでサイズオーバーと判定された場合はパスワードチェックせず、パスワード無しの判定とする
                LOG.warn("# ファイルサイズオーバー(ZIPファイル内）（fname:{}）", info.getFilePath());
                // メッセージ
                info.setErrMsg(errMsg.toString());
    ...
    protected boolean checkInput() {
        ...
        //無害化中ファイル送信有無の判定 [v2.2.6]
        if (!CommonUtil.isSectionLgwan()) { //Internet環境のみ
            if(checkInputTransferProcessing() == false){
                bret = false;       //無害化中ファイル送信あり
            }
        }
...
// チェックはファイル選択（アップロード）時にして処理済みのためここのチェックは不要[v.2.2.6]
...
//                        break;
    ...
    /**
     * 無害化中ファイル送信有無の判定  【v2.2.6】
     * @return true:無害化中送信なし。　false:無害化中送信あり
     */
    protected boolean checkInputTransferProcessing() {
        boolean bRet = true;
        try {
            LOG.debug(VerifyUtil.setLoggingMsg(VerifyUtil.VERIFY_CPU, "BEGIN"));
            ...
            if (commonBean.isLoginFlg()){
                ...
                //通常ログインの場合
                //--------------------------------
                mailAddress = commonBean.getMailAddress();
            }else{
                //--------------------------------
                //ワンタイムログインの場合、
                //--------------------------------
                //ワンタイムユーザID（onetimeId）からワンタイムユーザテーブルを検索し、メールアドレスを取得する
                OnceUser ou = onceUserService.find(commonBean.getOnetimeId());
                if(ou == null){
                    //ワンタイムユーザテーブルが無いので、当該ユーザに無害化中ファイル送信無しと判定
                    return true;
                }
                mailAddress = ou.getMailAddress();
            }
            
            //--------------------------------            
            //（２）メールアドレスでSendInfoを検索する
            //  ＜条件＞
            //      送信メールアドレス（sendmailaddress）が(1)で取得したメールアドレスと一致、且つ	
            //      メール添付フラグ（attachmentmailflg）がファイル交換（false）、且つ	
            //      登録日時（insertdate）が現在時刻より過去２時間以内
            //--------------------------------            
            Date dateTo = new Date();   //現在時刻
            //２時間前の時刻（オプション： 2時間以外としたい場合は、configの"FileSendProcessingCheckTime"（分）で設定可能）
            int checkMinute = itemHelper.findIntWithDefault(Item.SEND_FILE_TRANSFER_PROCESSING_CHECK_TIME, funcId, 120);    //configから取得（default=120分）
            if(checkMinute <= 0){
                //チェック時間として0以下が設定されていた場合は、チェック無しと判定
                return true;
            }
            Calendar cal = Calendar.getInstance();
            cal.setTime(dateTo);
            cal.add(Calendar.MINUTE, -checkMinute);
            Date dateFrom = cal.getTime();
            List<SendInfo> sendInfoDatas = sendInfoService.findSendFileProcessing(mailAddress, dateFrom, dateTo);
           
            //--------------------------------            
            // ３）SendInfoが見つかった場合、SendInfoのIDでUploadGroupInfoを検索する
            //  ＜条件＞
            //      送信情報ID（sendInfoId）が送信情報テーブルのＩＤ（id）と一致	
            //--------------------------------            
            for (SendInfo sendInfoData : sendInfoDatas) {
                List<UploadGroupInfo> ugiList = ugiManager.findUploadGroupInfoBySendInfoId(sendInfoData.getId());
                if(ugiList.size() > 0){
                    LOG.debug("#無害化中ファイル送信あり.  sendInfoId:{}, uploadGroupInfoId:{}", sendInfoData.getId(), ugiList.get(0).getId());
                    //当該ユーザに無害化中ファイル送信が存在
                    bRet = false;
                    break;
                }
            ...
            if(bRet == false){
                //無害化中ファイル送信がある場合
                //メッセージを表示してファイル送信不可とする
                FacesContext context = FacesContext.getCurrentInstance();
                //メッセージ：「無害化処理中のファイル送信があるため、連続してファイル送信を行うことができません。」
                String errMsg = itemHelper.findDispMessageStr(Item.ErrMsgItemKey.ERR_FILE_SEND_TRANSFER_PROCESSING, funcId);        
                context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "", errMsg));
            }
            return bRet;
        } catch (Exception e) {
            LOG.error("#! 無害化中ファイル送信有無の判定 で例外が発生しました。 errMsg:{}", e.toString(), e);
            return true;
        }finally{
            LOG.debug(VerifyUtil.setLoggingMsg(VerifyUtil.VERIFY_CPU, "END"));        
        }
    ...
    private List<String> getFileNameLengthOverMessages() {
        ...
        String errZipBomb = "";   //ZipBombファイル名（複数の場合は","+改行を先頭につけて連結）
        for (FileInfoBean info : fileInfoList) {
...
//            //TODO :::UT:::End
            ...
            switch(info.getFileErrorReason()){
                ...
                case FileNameLengthOverInZip:
                    ...
                    break;
                case ZipBomb:   
                    //高圧縮ZIPファイル（ZipBomb） [v2.2.5d]
                    if(errZipBomb.length()>0){
                        errZipBomb += ",\n";
                    }
                    errZipBomb += info.getFileName();
                    break;
                default:
            }
        }
        ...
        if(errZipBomb.length() > 0){
            //高圧縮ZIPファイル（ZipBomb）あり
            //ログ出力
            LOG.error("#!高圧縮ZIPファイルがあるためファイル送信に失敗しました。 (RegionId:{}, UserId:{}, OnetimeId:{}, zipFileName：{})"
                ,commonBean.getRegionId(), commonBean.getUserId(),  commonBean.getOnetimeId(), errZipBomb);
            //エラーメッセージ「高圧縮のＺＩＰファイルが含まれます、ＺＩＰファイルに含まれるファイル数をXXXファイル以下、ファイルサイズの合計をXXXＭＢ以下としてください。」
            //config設定値取得（ZIP内ファイルのファイル数上限値）
            int zipFilesCountLimit = itemHelper.findIntWithDefault(Item.ZIP_FILES_COUNT_LIMIT, Item.FUNC_COMMON, -1);       
            //config設定値取得（ZIP内ファイル解凍後ファイルサイズ上限値）
            int zipFilesSizeLimit = itemHelper.findIntWithDefault(Item.ZIP_FILES_SIZE_LIMIT, Item.FUNC_COMMON, -1);       
            resMsgs.add(itemHelper.findDispMessageStr(Item.ErrMsgItemKey.ERR_ZIP_BOMB_FILE, funcId, zipFilesCountLimit, zipFilesSizeLimit));
        }          
        return resMsgs;
    }
