    ...
    public boolean resetLoginLock(BasicUser basicuser)
    {
        ...
        if(basicuser.isLoginLockFlg()){
            ...
            if(lockClearHour > 0){
                ...
                if(isOverTime(basicuser.getPasswordFaultDate(), lockClearHour, Calendar.HOUR_OF_DAY)){
            ...
            if(basicuser.getPasswordFaultCount() > 0){
                ...
                if(isOverTime(basicuser.getPasswordFaultDate(), FAULTCNT_RESET_HOUR, Calendar.HOUR_OF_DAY)){
...
//        basicuser.setPassword(password);
     ...
     * @param srcDate   指定日付
     * @param addTime   追加時間   
     * @param calendarField 追加時間フィールド（時：Calendar.HOUR_OF_DAY/ 日：Calendar.DAY_OF_MONTH)
