...
//        File nfsDecryptDir = new File(CommonUtil.getSetting("decryptdir") + receiveInfo.getId());
        ...
        for (ReceiveFile receiveFile : receiveInfo.getReceiveFiles()) {
            ...
            SendFile sendFile = receiveInfo.getSendInfo().getSendFiles().stream()
...
//                FileUtil.deleteFolder(zipDir);
    ...
    private boolean createDecryptFiles(
        ...
        //ZipBombファイルかどうか [v2.2.5d]
        SendFile sendFile = receiveFile.getReceiveInfo().getSendInfo().getSendFiles().stream()
                .filter(s -> s.getId().equals(receiveFile.getSendFileId())).findFirst().orElse(new SendFile());
        boolean isZipBomb = sendFile.isZipBomb();
        VerifyUtil.outputUtLog(LOG, "#v2.2.5d#ZipBomb#", false, "IsZipBomb:%b, File:%s", isZipBomb, sendFile.getFileName());
        
        // Zipファイルの場合は中のファイル分も作成
        if (!isZipBomb && FileUtil.isZipFile(file) ) {
...
//TODO ファイル名長対応用                        decryptFile.setFileName(new File(fileName).getName());
    ...
    private DecryptFile createDecryptFile(
        ...
        if (file != null) {     // 実ファイルがある場合の設定
            ...
            if (!zipPwChkFlg && FileUtil.isZipFile(file)) {
                ...
                decryptFile.setPasswordFlg(FileUtil.checkPw(file, false, LOG, itemHelper));
            }            
        } else {                
            //実ファイル指定がない場合の設定
    ...
    private boolean getPasswordReEncryptFlg(ReceiveInfo recvInfo){
        ...
        VerifyUtil.outputUtLog(LOG, "#UT_v2.2.4#", false, "passwordReEncryptFlg:%s, mailAddress:%s", item.getValue(), recvInfo.getReceiveMailAddress());
