...
public class ManageIdBulkView extends ManageCommonView implements Serializable {
    ...
    /**
     * アップロードされたファイルのリスト
     * 最後に削除するために保持しておく[v2.2.3]
     */
    private List<File> uploadFileLst = new ArrayList<>();
...
//        String folder = CommonUtil.getSetting("tempdir") + "/" + IdUtil.createUUID();
        ...
        try (InputStream is = event.getFile().getInputstream()) {
            ...
            uploadFileLst.add(objFile); //リストに追加
        } catch (IOException e) {
            //例外処理
            e.printStackTrace();
    ...
    public void doPreDestroy(){
        ...
        //アップロードされたファイル(tempフォルダ内）を削除する
        uploadFileLst.forEach((f) -> {
            try {
                //ファイル削除
                if(f.exists()){
                    if(!f.delete()){
                        //削除不可でもException発生しないケースもあるため戻り値判定で削除結果を判定する。
                        LOG.warn("#!アップロード済みファイルの削除ができませんでした。 file:{}", f.getPath());
                    }
                }
                //フォルダ削除
                if(f.getParentFile().exists()){
                    if(!f.getParentFile().delete()){
                        //削除不可でもException発生しないケースもあるため戻り値判定で削除結果を判定する。
                        LOG.warn("#!アップロード済みファイルのフォルダ削除ができませんでした。 folder:{}", f.getParentFile());
                    }
                }                
            } catch (Exception e) {
                LOG.warn("#!アップロード済みファイルの削除処理に失敗しました。 file:{}, Err:{}", f.getPath(), e.toString(), e);
            }
        });
