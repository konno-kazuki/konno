...
public class HistoryLogic {
    ...
    @Getter
    @Setter
    protected String dspStatusExpiration;       //ステータス：有効期限切れ
    @Getter
    @Setter
    protected String dspStatusSanitize;         //ステータス：無害化処理中
    @Getter
    @Setter
    protected String dspStatusReceiveNone;      //ステータス：未受領
    @Getter
    @Setter
    protected String dspStatusReceive;          //ステータス：受領
    @Getter
    @Setter
    protected String dspStatusFileEmpty;        //ステータス：添付ファイル無
    @Getter
    @Setter
    protected String dspStatusDecrypting;       //ステータス：パスワード解除待ち
    @Getter
    @Setter
    protected String dspStatusDownloadAble;     //ステータス：ダウンロード可能
    @Getter
    @Setter
    protected String dspStatusDownloadComplete; //ステータス：ダウンロード完了
    @Getter
    @Setter
    protected String dspStatusCancel;           //ステータス：取り消し
    @Getter
    @Setter
    protected String dspStatusApproval;         //ステータス：承認待ち
    @Getter
    @Setter
    protected String dspStatusRejected;         //ステータス：却下済み
    @Getter
    @Setter
    protected String dspStatusApproved;         //ステータス：承認済み

    @Getter
    @Setter
    protected String dspStatusSendStarting;     //ステータス：送信中
    @Getter
    @Setter
    protected String dspStatusSendTransferErr;    //ステータス：送信処理失敗
    @Getter
    @Setter
    protected String dspStatusApproveRequestErr;  //ステータス：承認依頼処理失敗

    @Getter
    @Setter
    protected int approvedflgApproved;          //承認フラグ：承認
    @Getter
    @Setter
    protected int approvedflgRejected;          //承認フラグ：却下
    ...
    private String[] getStatusApprove(SendInfo sendInfo, List<ApproveInfo> approveInfos) {
        ...
        if (approveInfos != null) {
            ...
            for (ApproveInfo approveInfo : approveInfos) {
                if (approveInfo.getApprovedFlg() == approvedflgApproved) {
                    ///承認済数を計上
                    cntApproved++;
                } else if (approveInfo.getApprovedFlg() == approvedflgRejected) {
                    ///却下済数を計上
                    cntRejected++;
                }
            }
...
//            if (isConfigPasswordUnlockFlg && receiveInfo.isPasswordUnlockWaitFlg()) {
    ...
    public void cloneHistoryDataTable(DataTableBean base, DataTableBean to, String req) {
        ...
        to.setRowsPerPageTemplate(base.getRowsPerPageTemplate());
        to.setRows(base.getRows());
        to.setCurrentPage(base.getCurrentPage());
        to.setFirst(base.getFirst());
        to.setPaginator(base.isPaginator());
        to.setPaginatorTemplate(base.getPaginatorTemplate());
        to.setSelectionMode(base.getSelectionMode());
    ...
    public HistoryBean createApproveHistoryBean(SendInfo sendInfo, ApproveInfo approveInfo, Date sysDate) {
        ...
        if (isApproved || approveInfo.getApprovedFlg() == approvedflgApproved) {
            ///「承認する」不可、disabledRow
