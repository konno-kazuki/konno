...
public class SendHistoryDetailView extends HistoryCommonView implements Serializable {
    ...
    public void initFunc() {
        ...
        // 送信一覧から選択された送信情報ID
        String sendInfoId = sendHistoryBean.getUid();
        ...
        if ( StringUtils.isEmpty(sendInfoId) && StringUtils.isEmpty(commonBean.getOnetimeId()) ) {
            ...
            if (!sendHistoryDataTableBean.getReq().equals("")) {
                ...
                historyLogic.cloneHistoryDataTable(sendHistoryDataTableBean, save_historyDataTable, HistoryLogic.REQ_INIT);
            }
            // 送信情報を取得
        ...
        //SendHistoryBeanを作成
        sendHistoryBean = historyLogic.createSendHistoryBean(sendInfo, sysDate);
        sendHistoryBean.contentLineDispOn();

        //送信先情報（＝受信情報）
        if (sendHistoryBean.getReceiveInfos() == null) {
            receiveInfoList = historyLogic.findReceiveInfoForSendInfoId(sendInfo.getId());
        } else {
            receiveInfoList = sendHistoryBean.getReceiveInfos();
        }
    ...
    public void chgContent(ActionEvent actionEvent) {
        if (sendHistoryBean.isContentLineDisp()) {
            ///ライン表示→全表示
            sendHistoryBean.contentLineDispOff();
        } else {
            ///全表示→ライン表示
            sendHistoryBean.contentLineDispOn();
        }
    ...
    public void chgApproveContent(ActionEvent actionEvent) {
        if (sendHistoryBean.isApproveContentLineDisp()) {
            ///ライン表示→全表示
            sendHistoryBean.approveContentLineDispOff();
        } else {
            ///全表示→ライン表示
            sendHistoryBean.approveContentLineDispOn();
        }
    }
    ...
    public void actSendCancel(ActionEvent actionEvent) {
        ...
        try {
            ...
            //選択-送信履歴情報
            sendHistoryBean = (HistoryBean) actionEvent.getComponent().getAttributes().get("selectedRowData");
    ...
    public String actSendAction() {
        ...
        //HistoryDataTableBean値の複製
        //(save_historyDataTable→sendHistoryDataTableBean：リクエスト＝詳細画面から戻る)
        historyLogic.cloneHistoryDataTable(save_historyDataTable, sendHistoryDataTableBean, HistoryLogic.REQ_BACK_DETAIL);
