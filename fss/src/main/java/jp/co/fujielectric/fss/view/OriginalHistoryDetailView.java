...
public class OriginalHistoryDetailView extends HistoryCommonView implements Serializable {
    ...
    public void initFunc() {
        ...
        // 原本一覧から選択された原本情報ID
        String originalInfoId = injectHistoryBean.getUid();
        ...
        injectHistoryBean = historyLogic.createOriginalHistoryBean(receiveInfo, sysDate);
...
//                try {
//                    downloadFileName = new String((injectHistoryBean.getSubject() + ".zip").getBytes("Shift_JIS"), "ISO-8859-1");  // 件名 + .zip
...
//        }
    ...
    public void eventDownloadEml() {
        ...
        try {
            ...
            //----------------------------------------------------
            //メールヘッダからX-EnvelopeTo,Fromを削除する [v2.2.4e]
            //----------------------------------------------------
            try {
                inputStream = mailManager.getMailInputStreamWithDeleteXenv(_file);                
            } catch (Throwable e) {
                LOG.error("#! メールヘッダのX-Envelope情報削除処理に失敗しました。", e.toString());
                //例外発生時は、オリジナルメールそのままを対象とする。
                inputStream = new FileInputStream(_file);
            }
